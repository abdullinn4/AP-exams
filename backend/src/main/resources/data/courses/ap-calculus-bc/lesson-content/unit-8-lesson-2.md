# **Lesson 20: Volume of Solids**

### **20.1 Volumes by Cross Sections**
...
1. **Square Cross Sections:** The base of a solid is the region bounded by $y = \sqrt{x}$, $y = 0$, and $x = 4$. Cross sections perpendicular to the x-axis are squares. Find the volume.
...
2. **Semicircular Cross Sections:** The base is the same region as above. Cross sections perpendicular to the x-axis are semicircles with their diameters lying in the base. Find the volume.
...
3. **Triangular Cross Sections (Isosceles Right):** The base is the region in the first quadrant bounded by $y = 4 - x^2$, $x=0$, and $y=0$. Cross sections perpendicular to the y-axis are isosceles right triangles with hypotenuse in the base. Find the volume.
    - **Comment:** Since cross sections are perpendicular to the y-axis, we integrate with respect to $y$.
    ...
    - **Volume:** $V = \int_{0}^{4} \frac{4-y}{4} \, dy = \frac{1}{4} \left[ 4y - \frac{y^2}{2} \right]_{0}^{4} = \frac{1}{4} \left( 16 - 8 \right) = \frac{1}{4} \cdot 8 = 2$.

---

### **20.2 Disc Method (Solids of Revolution)**
...
1. **Revolving around the x-axis:** Find the volume of the solid generated by revolving the region bounded by $y = x^2$, $y=0$, and $x=2$ about the x-axis.
    - **Radius:** $R = x^2$.
    - **Volume:** $V = \pi \int_{0}^{2} (x^2)^2 \, dx = \pi \int_{0}^{2} x^4 \, dx = \pi \left[ \frac{x^5}{5} \right]_{0}^{2} = \pi \cdot \frac{32}{5} = \frac{32\pi}{5}$.
2. **Revolving around a horizontal line $y = k$:** The region bounded by $y = \sqrt{x}$, $y=0$, and $x=4$ is revolved about the line $y = -1$. Find the volume.
...
1. **Basic Washer:** Find the volume of the solid generated by revolving the region bounded by $y = x^2$ and $y = \sqrt{x}$ about the x-axis.
...
2. **Revolving around a vertical line:** The region bounded by $y = x^3$, $y=0$, and $x=1$ is revolved about the line $x=2$. Find the volume.
    ...
    - **Radii:** The region is to the left of $x=2$. Distance from curve $x=\sqrt[3]{y}$ to axis $x=2$: $R = 2 - \sqrt[3]{y}$. The inner radius? There is no hole because the region extends to the y-axis ($x=0$), which is closer to $x=2$. Wait, the region is between $x=0$ and $x=1$. When revolved, for a given $y$, the outer boundary is at $x=\sqrt[3]{y}$, but the inner boundary is at $x=0$? No, careful: The region is bounded on the left by $x=0$ (the y-axis). When revolved around $x=2$, the solid will have a hole from $x=0$ to the curve. So we have a washer.
        - **Outer radius:** Distance from the **leftmost** part of the region (which is $x=0$) to $x=2$: $R = 2 - 0 = 2$.
        - **Inner radius:** Distance from the curve $x=\sqrt[3]{y}$ to $x=2$: $r = 2 - \sqrt[3]{y}$.
...
### **20.4 Choosing a Method & Calculator Techniques**
...
3. **Revolve about x-axis:** Region bounded by $y=x^2$ and $y=4$. Intersections: $x^2=4$ → $x=\pm 2$. Washer method: Outer radius $R=4$, inner radius $r=x^2$.
...
4. **Revolve about y-axis:** Region: $y=x^2$ from $x=0$ to $x=2$, $y=0$ to $y=4$. Using disc method w.r.t. $y$: For $y$ from 0 to 4, radius is $x = \sqrt{y}$. But careful: The region is bounded on the right by $x=2$. For $y$ from 0 to 4, the horizontal slice goes from $x=\sqrt{y}$ to $x=2$? Wait, when revolving about the y-axis, the solid is generated by the region between the curve $x=\sqrt{y}$ and the line $x=2$? Actually, the region is bounded by $y=x^2$, $y=0$, and $x=2$. When revolved about the y-axis, the part from $x=0$ to $x=\sqrt{y}$ is included? Let's think: For a given $y$, the region extends from $x=\sqrt{y}$ to $x=2$. But when revolving about the y-axis, the radius is the x-distance. The outer radius is 2, the inner radius is $\sqrt{y}$. Washer method!
    - Outer radius: $R = 2$.
    - Inner radius: $r = \sqrt{y}$.
    - $V = \pi \int_{0}^{4} (4 - y) \, dy = \pi \left[ 4y - \frac{y^2}{2} \right]_{0}^{4} = \pi (16 - 8) = 8\pi$.
5. **Revolve about x-axis:** Region bounded by $y=x^2$ and $y=2x$. Intersections: $x^2=2x$ → $x(x-2)=0$ → $x=0, 2$. On [0,2], $2x \ge x^2$. Washer: Outer radius $R=2x$, inner radius $r=x^2$.
...
6. **Revolve about $y=-1$:** Region: $y=x^3$, $y=0$, $x=1$. Disc method w.r.t. $x$: Radius = distance from curve to axis: $x^3 - (-1) = x^3+1$.
...
7. **Revolve about $x=4$:** Region: $y=\sqrt{x}$, $y=0$, $x=4$. Using cylindrical shells might be easier, but we haven't covered that yet. Let's use washer method w.r.t. $y$. Rewrite: $y=\sqrt{x}$ → $x=y^2$. y from 0 to 2. When revolving about $x=4$, the outer radius is distance from $x=0$ to $x=4$? Wait, the region is bounded on the left by $x=0$, on the right by $x=4$? Actually, the region is under $y=\sqrt{x}$ from $x=0$ to $x=4$. For a given $y$, the horizontal slice goes from $x=y^2$ to $x=4$. When revolved about $x=4$, the solid has a hole. The outer radius is distance from the left boundary $x=0$ to $x=4$: $R=4-0=4$. The inner radius is distance from $x=y^2$ to $x=4$: $r=4-y^2$.
$V = \pi \int_{0}^{2} [4^2 - (4-y^2)^2] \, dy = \pi \int_{0}^{2} (16 - (16 - 8y^2 + y^4)) \, dy = \pi \int_{0}^{2} (8y^2 - y^4) \, dy = \pi \left[ \frac{8y^3}{3} - \frac{y^5}{5} \right]_{0}^{2} = \pi \left( \frac{64}{3} - \frac{32}{5} \right) = \pi \left( \frac{320}{15} - \frac{96}{15} \right) = \frac{224\pi}{15}$.
8. **Semicircular cross sections perpendicular to y-axis:** Base: enclosed by $y=x^2$ and $y=3$. Intersections: $x^2=3$ → $x=\pm\sqrt{3}$. For $y$ from 0 to 3, the horizontal width is from $x=-\sqrt{y}$ to $x=\sqrt{y}$, so diameter = $2\sqrt{y}$. But wait, the base is the entire region between the parabola and the line $y=3$. Actually, the region is bounded by $y=x^2$ below and $y=3$ above. For a given $y$ between 0 and 3, the horizontal slice goes from $x=-\sqrt{y}$ to $x=\sqrt{y}$, so length = $2\sqrt{y}$. That is the diameter of the semicircle.
    - Diameter: $d = 2\sqrt{y}$.
    - Area: $A(y) = \frac{1}{2} \pi \left( \frac{d}{2} \right)^2 = \frac{1}{2} \pi \left( \frac{2\sqrt{y}}{2} \right)^2 = \frac{1}{2} \pi (\sqrt{y})^2 = \frac{\pi}{2} y$.
    - Volume: $V = \int_{0}^{3} \frac{\pi}{2} y \, dy = \frac{\pi}{2} \left[ \frac{y^2}{2} \right]_{0}^{3} = \frac{\pi}{2} \cdot \frac{9}{2} = \frac{9\pi}{4}$.