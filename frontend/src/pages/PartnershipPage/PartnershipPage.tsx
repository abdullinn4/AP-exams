...
import { PARTNERSHIP_CONTENT } from '@/shared/config/content/partnership.content'
import {useSubscribeToPartnershipMutation} from "@/shared/api/partnershipApi.ts";
...
export const PartnershipPage = () => {
    ...
    const [subscribeToPartnership, { isLoading, isError }] = useSubscribeToPartnershipMutation()

    const handleSubmit = async (e: FormEvent) => {
        e.preventDefault()

        try {
            await subscribeToPartnership({ email }).unwrap()
            setIsSubmitted(true)
            setEmail('')
            ...
            <section className="section-card-padding" style={{ paddingBottom: '100px' }}>
                <div className="w-layout-blockcontainer container-default width-100 w-container">
                    <div className="w-layout-grid grid-2-columns newsletter-grid">
                        ...
                        >
                            <img
                                src={PARTNERSHIP_CONTENT.hero.image}
                                loading="eager"
                                alt="Subscribe to Our Weekly Newsletter - SmashAP"
                        ...
                        >
                            <div className="inner-container _390px _100-tablet">
                                <h1 className="display-6">{PARTNERSHIP_CONTENT.hero.title}</h1>
                            </div>

                            <div className="mg-top-12px">
                                <p style={{fontSize: '17px'}}>{PARTNERSHIP_CONTENT.hero.description}</p>
                            </div>

                            {/* Features */}
                            <div className="mg-top-24px">
                                <div className="w-layout-grid grid-1-column gap-row-16px">
                                    {PARTNERSHIP_CONTENT.features.map((feature: Feature) => (
                                        <div
                                            key={feature.id}
                                            id={`w-node-${feature.id}`}
                                            className="pricing-feature-wrapper"
                                        >
                                            {/* Иконка вместо check-icon */}
                                            <div style={{
                                                width: '24px',
                                                height: '24px',
                                                flexShrink: 0,
                                ...
                                <div className="form w-form">
                                    ...
                                    >
                                        <div className="input-wrapper">
                                            <input
                                                ...
                                                maxLength={256}
                                                name="Email"
                                                placeholder={PARTNERSHIP_CONTENT.form.placeholder}
                                                type="email"
                                                id="Email"
                                                required
                                                value={email}
                                            ...
                                            <input
                                                ...
                                                value={isLoading ? 'Submitting...' : PARTNERSHIP_CONTENT.form.submitText}
                                                disabled={isLoading}
                                            />
                                        </div>
                                    </form>
                                    ...
                                    {isSubmitted && (
                                        <div className="success-message-wrapper w-form-done" style={{ display: 'block' }}>
                                            <div className="card message inside-input">
                                                <div className="icon-font-rounded"></div>
                                                <div className="text-neutral-800">
                                                    <div className="display-1">
                                                        {PARTNERSHIP_CONTENT.form.successMessage}
                                                        <br />
                                                    </div>
                                                </div>
                                            </div>
                                    ...
                                    {/* Error Message */}
                                    {isError && (
                                        <div className="error-message-wrapper w-form-fail" style={{ display: 'block' }}>
                                            <div>{PARTNERSHIP_CONTENT.form.errorMessage}</div>
                                        </div>
                                    )}
                                </div>
                            </div>
